...
def train_epoch(
    ...
    for batch_idx, batch in enumerate(pbar):
        ...
        if hasattr(model, 'forward'):
            ...
            # Handle LSTM returning tuple
            if isinstance(output, tuple):
                output = output[0]
        
        # Compute loss
        loss = model.compute_loss(output, Y, mask)
...
def evaluate(
    ...
    for batch in tqdm(dataloader, desc="Evaluating"):
        ...
        if use_rollout:
            # Extract context (game state) from first timestep
            context_dim = model.get_context_dim()
            init_context = X[:, 0, :context_dim]
            
            # Get initial positions from first timestep of Y
            init_positions = Y[:, 0, :]
            
            # Determine horizon from max sequence length in batch
        ...
        # Compute loss
        loss = model.compute_loss(pred, Y[:, :pred.shape[1]], mask[:, :pred.shape[1]])
        total_loss += loss.item()
        
        # Compute ADE/FDE
        # Reshape for position-based metrics: assume first 2 dims of each player are x, y
        # For now, compute on flattened output
        ade = compute_ade(pred.cpu().numpy(), Y[:, :pred.shape[1]].cpu().numpy())
        fde = compute_fde(
            pred.cpu().numpy(), 
            Y[:, :pred.shape[1]].cpu().numpy(),
            mask[:, :pred.shape[1]].cpu().numpy()
        )
...
def train_model(
    ...
    # Build model
    model = build_model(model_type, input_dim, output_dim, config)
...
def main():
    ...
    output_dim = datasets['train'].output_dim
    ...
    for model_type in models_to_train:
        model, history = train_model(
            model_type, config, device, dataloaders, input_dim, output_dim
        )
